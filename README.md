...
If `GOOGLE_CALENDAR_ID` is *already* set when you run it, `create_calendar.py` doesn't create a new calendar at all — it just ensures the metadata spreadsheet and its tabs for that already-configured calendar instead (useful if you set this app up before the spreadsheet, or one of its tabs, existed). Either way, this is a one-time, human-run bootstrap step just like the calendar itself — there's deliberately no MCP tool or CLI command for it (see [MCP tools](#mcp-tools) and [Calendar metadata sheets](#calendar-metadata-sheets) below); creating the event labels tab fails if that calendar already has one tracked.
