...
| `create_noted_time` | `(noted_time: NotedTime) -> NotedTime` |
...
The noted-times tab is simpler, since a noted time has no Calendar API counterpart to reconcile with: [`utilities/noted_time_sheet.py`](utilities/noted_time_sheet.py)'s `NotedTimeSheet` (`NotedTimeSheet.ensure` finds or creates/tags the tab, with just its header row, the same way `EventLabelSheet.ensure` does) is what `server.py`/`calendar_cli.py` talk to directly — no `EventLabels`-equivalent layer above it. `create_noted_time`/`note` (see [MCP tools](#mcp-tools) above and [Command-line utilities](#command-line-utilities) below) call `NotedTimeSheet.append`, which also creates the spreadsheet/tab on the fly if neither exists yet, the same as the event-label tools.
