...
1. **`prepare_compaction`** (read-only) returns one *reallocation day* of uncompacted notes — each with a permanent id (`n5`: its sheet row) and a shortlist of nearby planned events as `candidates` — plus that day's planned events and instructions for the next step. A backlog spanning several days takes one round per day, oldest first.
...
- One with only a start note runs until the *next* note that starts or ends something, so gaps are filled by extending it rather than left empty. The last one is still in progress: it runs to now, or its planned end if that's later, but never into the end-of-day sleep block.
...
- Activities that come out overlapping are **merged into one event** rather than guessing — "9:00 started email", "9:30 done with report" becomes a single "Email and Report" from 9:00 to 9:30. The first planned one keeps its id; another planned one is cancelled.
- `marker` text is appended to the description of the event it falls inside, so it survives compaction.
...
- **Noted Times** — time notes: three columns, **Timestamp**, **Description** (optional) and **Compaction ID** (blank until the note has been compacted; see [Compacting notes](#compacting-notes)). Rows are never deleted, so a row number is a permanent note id.
