...
The noted-times tab is simpler, since a noted time has no Calendar API counterpart to reconcile with: [`utilities/noted_time_sheet.py`](utilities/noted_time_sheet.py)'s `NotedTimeSheet` (`NotedTimeSheet.ensure` finds or creates/tags the tab, with just its header row, the same way `EventLabelSheet.ensure` does) is what `server.py`/`calendar_cli.py` talk to directly — no `EventLabels`-equivalent layer above it. The `note` MCP tool and the `note` CLI command (same name, different surfaces — see [MCP tools](#mcp-tools) above and [Command-line utilities](#command-line-utilities) below) both call `NotedTimeSheet.append`, which also creates the spreadsheet/tab on the fly if neither exists yet, the same as the event-label tools.
...
`note` takes a positional `timestamp` (an ISO 8601 datetime with a UTC offset, same format as `start`/`end` above -- required) and an optional positional `description` (quote it if it contains spaces), builds a `utilities/noted_time_sheet.py` `NotedTime` from them, and appends it to this calendar's noted-times tab via `NotedTimeSheet.append` — see [MCP tools](#mcp-tools) and [Calendar metadata sheets](#calendar-metadata-sheets) above. There's no `list`/`delete` command for notes yet; open the sheet directly for that.
