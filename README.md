...
- [`utilities/reallocating_calendar.py`](utilities/reallocating_calendar.py) — the glue between the two above: `ReallocatingCalendar` wraps a `CalendarClient` with reallocation-aware `create_event`/`update_event`, the shared entry point both `server.py` and `calendar_cli.py` use. This is application-level policy (how creating/moving an event makes room for itself), not Calendar API integration, which is why it isn't a `CalendarClient` method.
