...
- [`utilities/memory_diagnostics.py`](utilities/memory_diagnostics.py) — `track(label)`, a context manager wrapping an operation (every MCP tool, `load_credentials`) to log RSS, `tracemalloc` growth (only if something else has already started it), and `objgraph` object-count growth since the previously tracked operation — for narrowing down what's driving this process's memory usage.
